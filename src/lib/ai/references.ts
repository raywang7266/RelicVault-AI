...
import type { WebSearchResult } from "@/lib/ai/zhipu-chat";
...
export interface ChatReference {
   ...
   *  网络来源若检索结果无 link，则退化为「标题搜索」链接（isSearch=true）。 */
  url: string;
...
/**
 * 退化为「标题搜索」链接：智谱 web_search 的 link 字段经常为空，
 * 此时用标题构造一个搜索引擎查询，保证来源仍可点击溯源（非原始文章地址）。
 */
function searchUrlFor(title: string): string {
  const q = clip(title, 60) || "文物";
  return `https://www.baidu.com/s?wd=${encodeURIComponent(q)}`;
}

/** 把智谱联网搜索结果转成参考资料（校验 / 去重 / 截断）。
 *  注意：link 为空时不再丢弃，而是退化为「标题搜索」链接，
 *  以此保证联网检索到的来源总能展示出来（修复「时灵时不灵」）。 */
export function webReferences(
  ...
  for (const item of results) {
    ...
    const host = isHttp ? hostnameOf(rawLink) : "";
    const url = isHttp ? rawLink : title ? searchUrlFor(title) : "";
    if (!url) continue; // 既无链接也无标题的条目跳过
