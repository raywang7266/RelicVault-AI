...
A crowdsourced digital heritage museum powered by AI. Contributors upload a photo of an artifact, Zhipu GLM vision suggests a title / era / category / description and auto-tags it, and the entry is stored in MongoDB together with GPS coordinates. Exact coordinates are **never** returned to the client — the API only serves blurred ones, so unprotected heritage sites cannot be pinpointed. A built-in **AI chat assistant (文遗小助手)**, also powered by Zhipu GLM, answers heritage questions, searches the web, and cites its sources — right from a floating widget on every page.

- 🖼️ **AI-assisted submission** — one click fills in title, era, category, description and tags from a photo (one or several photos, up to 6)
- 💬 **AI chat assistant (文遗小助手)** — a floating widget powered by Zhipu GLM `glm-4-flash`. It understands the page you're on, can do **web search**, and — after every answer — shows **cited references** (real search results + on-site links), never fabricated URLs.
...
- ❤️ **Social layer** — likes, favorites, comments, follows and red-dot notifications
- 🌓 **Day / night theme** — one-click toggle; preference is persisted and applied before first paint (no flash)
- 🔠 **Adjustable text size** — 6-level accessibility control that scales *only* the text, never the layout or spacing
...
| AI vision | [Zhipu GLM](https://open.bigmodel.cn) `glm-4v-flash` (image analysis, China-direct, free tier) |
| AI chat | [Zhipu GLM](https://open.bigmodel.cn) `glm-4-flash` (assistant text + web search, same free tier) |
...
- **Crowdsourced artifact archive** — upload photos (up to 6 per entry; the first is the cover), let the multimodal AI (Zhipu GLM) auto-identify era, category and tags, then record folk relics & heritage.
...
- **AI chat assistant (文遗小助手)** — a floating widget in the bottom-right corner:
  - **Three states** — `mini` (a draggable floating button), `expanded` (a chat dialog), and `hidden` (dismissed; a "summon" button reappears in the navbar).
  - **Page-aware** — it knows which page you're on (home / explore / upload / artifact detail / profile …) and reads the visible page text, so it can answer "what is this artifact?" about the one you're viewing.
  - **Web search** — built on Zhipu GLM `glm-4-flash` with the platform's `web_search` tool enabled, so it can pull in up-to-date info (exhibitions, news, prices, research).
  - **Suggested questions (联想词)** — when you open the dialog it offers 4–6 page-relevant question chips; generated by AI, with an offline fallback if the model is unreachable.
  - **Cited references (参考资料)** — every answer ends with a list of sources. Web sources come from the *real* search results (never hallucinated links); when a result has no URL it degrades to a title-search link so the source is still traceable. On-site links point to related pages.
  - **Resilient errors** — timeout / rate-limit (429) / network / generic failures are classified and shown in the user's language instead of a silent failure.
- **Day / night theme** — toggle in the navbar (sun/moon). The choice is stored in `localStorage["rv_theme"]` and applied by an inline script before first paint to avoid a flash. Brand colors (parchment / bronze / warm gold) flip with the theme; accent colors stay constant so white-on-button contrast never breaks.
- **Adjustable text size** — an accessibility control (type icon) with 6 levels (小 0.875× → 极大 1.9×). It sets a `--font-scale` variable that affects *only* font sizes; layout, spacing, icons and borders stay fixed, so the site never degrades into a whole-page zoom.
...
| `ZHIPU_CHAT_MODEL` | optional, default `glm-4-flash` | Model for the chat assistant (text) |
| `ZHIPU_WEB_SEARCH` | optional, default `true` | Set `false` to disable the assistant's web search |
...
│   │   ├── assistant/          # floating chat widget (文遗小助手) + context
│   │   ├── a11y/               # theme toggle · font-size control
│   │   ├── ai/                 # AI analysis panel
│   │   ├── auth/ · profile/ · layout/ · ui/
│   ├── hooks/                  # use-user · use-artifacts · use-artifact-submit
│   ├── lib/
│   │   ├── ai/                 # zhipu-chat · references · page-context · capture-page
...
| POST | `/api/analyze-artifact` | Send a photo (or photos) to Zhipu GLM, get metadata suggestions |
| POST | `/api/ai/chat` | Chat with the assistant (page context + web search + references) |
| POST | `/api/ai/suggest` | Generate page-aware suggested questions (chips) |
| GET | `/api/following` | Current user's followed-people avatar strip |
| GET | `/api/notifications` | Unread red-dot counts + notification list |
| GET/POST/DELETE | `/api/artifacts/[id]/comments/[commentId]/like` | Like / unlike a comment |
| GET | `/api/artifacts/[id]/image` | Serve an artifact image stored in the DB (data URL) |
| GET | `/api/users/[id]` | Public profile (collection + stats) |
| POST | `/api/users/[id]/follow` | Follow / unfollow a user |

> `/api/upload` and `/api/ai/analyze` exist as **501 placeholders** (not yet implemented).
...
**`Artifact`** — `title`, `description`, `imageUrl` (cover = `images[0]`, kept for backward-compat), `images[]` (up to 6 data URLs), `aiTags[]`, `manualTags[]`, `era`, `dynasty`, `category`, `locationName`, `preservationStatus`, `exactLat` / `exactLng` (**server-only**), `blurredLat` / `blurredLng`, `isProtected`, `userId`, `likes[]`, `favorites[]`, `comments[]`, `createdAt`.
...
- The **AI assistant** reads the current page's URL + visible text to build its context (`src/lib/ai/page-context.ts`, `capture-page.ts`). Web search uses Zhipu's `web_search` tool; "references" shown after each reply are the *real* retrieved sources — when a result has no URL it degrades to a title-search link so the source stays traceable, and links are never invented by the model.
- **Theme & text size** are pure client-side preferences (`localStorage["rv_theme"]` / `["rv_font_scale"]`) applied by an inline `<head>` script before first paint. The font-scale variable affects only `font-size` tokens, never spacing or chrome, so the layout never reflows.
